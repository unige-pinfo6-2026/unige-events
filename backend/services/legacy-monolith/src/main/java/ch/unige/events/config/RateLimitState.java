 ...
 * trigger.
 *
 * <p>Caffeine evicts entries idle for >5 minutes, so cold/sporadic users don't
 * pin memory. Recreating a bucket on next call is behaviourally equivalent to
 * a fresh window — exactly what we want for cold users.
 *
...
public class RateLimitState {
     ...
     * windowMillis}, the bucket starts a fresh window with count = 1. Otherwise
     * the count is incremented; if it exceeds {@code max}, the call is rejected
     * and {@code retryAfter = windowEnd - now} (rounded up to whole seconds).
     */
