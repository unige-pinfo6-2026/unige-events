...
/**
 * Read-only stub of the User entity (id + auth0_id only). view-service
 * needs to resolve the JWT's auth0 sub claim → users.id (UUID) to record
 * a view. Full User entity lives in user-service (PR 12) — at that point
 * this stub is replaced by a REST client to user-service.
 */
@Entity
@Table(name = "users")
