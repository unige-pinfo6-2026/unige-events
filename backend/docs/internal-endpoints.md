...
| 11 | `GET /users/_internal-followed-ids?followerId={uuid}` | user-service | event-service (`EventResource.getAll` — filtre `followedOnly`) | `List<UUID>` | SCRUM-168 Sprint 9. Retourne les UUIDs des utilisateurs suivis par `followerId` avec statut ACCEPTED. `followerId` null → `[]`. Utilisé par event-service pour construire la condition JPQL `e.creatorId IN :followedIds` sur `GET /events?followedOnly=true`. `@PermitAll` + `@Internal`. Fallback du REST client retourne `[]` (dégradation gracieuse : le feed affiche une liste vide plutôt qu'une erreur 503). |
