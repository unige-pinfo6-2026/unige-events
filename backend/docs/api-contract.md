...
| `GET` | `/events` | `@PermitAll` | Liste paginée — filtres : status, category, organizerId, endDateFrom (date-time), faculty | 200 |
| `POST` | `/events` | `@Authenticated` | Créer un événement | 201 |
| `GET` | `/events/search` | `@PermitAll` | Recherche full-text (q, category, faculty, dateFrom, dateTo, page, size) | 200 |
