...
| `GET` | `/users/search` | user-service | `@Authenticated` + `@PerUserRateLimit(name="users.search", max=60)` | SCRUM-137 polish — prefix scan sur `username` pour l'autocomplete d'invitation co-organisateur. Query params `q` (2-30 chars, `[a-zA-Z0-9._-]`) et `limit` (1-20, default 8). Renvoie un tableau de `UserPublicResponse` (projection `fromAnonymous` — id, username, displayName, avatarUrl, profilePublic). L'appelant est exclu des résultats (anti self-invite). | 200, 400, 401, 429 |
