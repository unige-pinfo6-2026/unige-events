...
import { type User, type UserPublicResponse } from '@/types/user'
...
function PublicProfileView({ profile, isMeRoute, canFollow, onProfileMutated, statsLoading = false }: Readonly<PublicProfileViewProps>) {
  ...
  const leftExtras = (
    <>
      {(facultyName || interests.length > 0) && (
        <div className="flex flex-col gap-3">
          {facultyName && <PropertyRow icon={GraduationCap}>{facultyName}</PropertyRow>}
          {interests.length > 0 && (
