...
interface ProfilePrivateStateProps {
  /**
   * Projection verrouillée retournée par le backend pour un profil privé
   * (id, username, displayName, avatarUrl, bannerUrl, compteurs). On garde le
   * même cadre visuel qu'un profil public (bannière + avatar overlapping +
   * displayName + compteurs) pour signaler qu'il s'agit d'un vrai compte,
   * simplement verrouillé. Quand absent (user introuvable), on retombe sur un
   * banner gradient générique sans avatar ni displayName.
   */
 ...
 * displayName + compteurs — puis remplace toute la zone de contenu par un
 * grand cadenas centré accompagné du texte « Compte privé ». Quand `canFollow`
 * est vrai et que la projection verrouillée contient l'`id`, un FollowButton
