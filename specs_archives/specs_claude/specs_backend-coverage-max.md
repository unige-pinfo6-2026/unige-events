...
### 8.1 ✅ BUG APPLICATIF DÉTECTÉ puis CORRIGÉ (accord PO — exception unique à « 0 src/main »)
**`ReportService.listByStatus` levait une `NullPointerException` quand une page de signalements ne contenait QUE des reports « comment-only ».** `bulkFetchEvents` renvoyait `Map.of()` (map **immuable**) quand `eventIds` était vide ; or `listByStatus` fait `eventsById.get(r.eventId)` et `r.eventId == null` pour un report comment-only → `Map.of().get(null)` **lève NPE** (les maps immuables refusent la clé null, contrairement à `HashMap`). Impact prod : 500 pour l'admin si une page de modération ne liste que des reports de commentaire. **Bug pré-existant sur `main`** (non introduit par cette PR ; révélé par la couverture). **Correctif (1 ligne, validé PO) : `bulkFetchEvents` retourne `new HashMap<>()` pour le cas vide** → lookup à clé null sûr. **Seule modif `src/main` de la PR.** Le test de régression `listByStatus_allCommentOnlyReports_returnsListWithoutNpe` prouve le correctif **et** couvre désormais **308/309** (garde set-vide) + branche **236** (`eventId==null`) → ces lignes **sortent du plafond**.

### 8.2 Plafond final atteint (25 branches + 5 lignes après correctif §8.1 — tout justifié, ZÉRO exclusion)
